...
for i=1:rb
    lmiterm([q 1 1 W1],A,1,'s');
    lmiterm([q 1 1 W2],B(:,:,i),1,'s');
...
for i=1:rb    
    ...
    % Setor Cônico
    q = q + 1;
    lmiterm([q 1 1 W1],sind(theta)*A,1,'s');
    lmiterm([q 1 1 W2],sind(theta)*B(:,:,i),1,'s');
    lmiterm([q 1 2 W1],cosd(theta)*A,1);
    lmiterm([q 1 2 W1],-cosd(theta),A');
    lmiterm([q 1 2 W2],cosd(theta)*B(:,:,i),1);
    lmiterm([q 1 2 -W2],-cosd(theta),B(:,:,i)');
    lmiterm([q 2 2 W1],sind(theta)*A,1,'s');
    lmiterm([q 2 2 W2],sind(theta)*B(:,:,i),1,'s');
